 ...
 *
 * <p>
 * This class provides concrete database operations for the {@code Laptop}
 ...
 *
 * <p>
 * All database interactions are performed using JDBC, and connections are
 * managed through the {@code DatabaseConnection} utility class.
 * </p>
 *
 * <p>
 * This implementation follows the {@code LaptopDao} interface, ensuring a
 * consistent contract for all laptop-related persistence operations.
 * </p>
 *
...
public class LaptopDaoImpl implements LaptopDao {
     ...
     * @author Sugam Rana Magar
     *         Deletes a laptop record from the database based on the provided
     *         laptop ID.
     *
     *         <p>
     *         This method establishes a database connection, prepares a SQL DELETE
     *         statement, and executes it to remove the corresponding laptop entry
     *         from the <code>Laptop</code> table.
     *         </p>
     ...
     *                   {@link SQLException} encountered is caught and logged
     * @author Sugam Rana Magar
     *         Retrieves a {@code Laptop} record from the database based on its
     *         unique ID.
     *
     *         <p>
     *         This method establishes a database connection, executes a
     *         parameterized
     *         SQL SELECT query, and maps the resulting row to a {@code Laptop}
     *         object
     *         using the <code>laptopAssginer</code> method.
     *         </p>
     * @implNote The database connection is closed in the <code>finally</code>
     ...
     *                   {@link SQLException} encountered is caught and logged
     * @author Sugam Rana Magar
     *         Retrieves a {@code Laptop} record from the database using its unique
     *         UUID.
     *
     *         <p>
     *         This method establishes a database connection, executes a
     ...
     *         {@code Laptop} object via the <code>laptopAssginer</code> method.
     *         </p>
     * @implNote The database connection is closed in the <code>finally</code>
     *           block to ensure proper resource management and prevent leaks.
     */
     ...
     *
     *         <p>
     *         This method establishes a database connection, prepares an SQL UPDATE
     *         statement, and sets all fields of the laptop based on the values
     *         provided
     ...
     *
     *         <p>
     *         This method establishes a database connection, executes a SQL SELECT
     *         query to fetch all rows from the <code>laptop</code> table, and maps
     *         each
     *         row to a {@code Laptop} object using the <code>laptopAssginer</code>
     *         method.
     *         The results are collected into an {@code ArrayList}.
     *         </p>
    ...
    public ArrayList<Laptop> fetchAllLaptops(int start, int limit) {
        ...
        Connection conn = null;
        String sql = "SELECT * FROM laptop ORDER BY laptopID LIMIT ?,?"; /*
                                                                          * RAND() aggregate function to randomize the
                                                                          * output when rendering all laptops
                                                                          */
        try {
            conn = DatabaseConnection.getConnection();
            PreparedStatement stmt = conn.prepareStatement(sql);
     ...
     *                   {@link SQLException} encountered is caught and logged
     * @author Sugam Rana Magar
     *         Retrieves a filtered list of {@code Laptop} records based on search
     *         criteria.
     *
     *         <p>
     *         This method dynamically builds and executes an SQL query using the
     *         provided
     *         filters: search keyword, brand, category, and price range. The search
     *         is
     *         case-insensitive and matches against multiple fields such as brand,
     *         description, title, and model.
     *         </p>
     *
     *         <p>
     *         The {@code priceCondition} parameter determines the price range:
     *         <ul>
     ...
     *
     *         <p>
     *         All string filters use SQL LIKE patterns generated by the
     *         <code>toLikeValue</code> method.
     *         </p>
     * @implNote The database connection is closed in the <code>finally</code>
     *           block to ensure proper resource management and prevent leaks.
     */
    @Override
    public ArrayList<Laptop> getLaptopsFilterSearch(String searchWord, String brand, String category,
            String priceCondition, int start, int limit) {
        ArrayList<Laptop> laptops = new ArrayList<>();
        Connection conn = null;
     ...
     *
     * <p>
     * This method applies filters for brand, category, keyword search, and
     * price range. The price range is determined by the {@code priceCondition}
     * parameter.
     * </p>
     *
     * @param searchWord     keyword used for searching across multiple fields
     * @param brand          brand filter (case-insensitive, partial match)
     * @param category       category filter (case-insensitive, partial match)
     * @param priceCondition price range selector (1–5)
     * @return the total number of matching laptops, or {@code 0} if an error occurs
     */
    @Override
    ...
    /**
     * Retrieves the number of laptops grouped by predefined categories.
     *
     * <p>
     * This method executes a COUNT query for each category and stores the
     * results in a {@code HashMap}, where the key is the category name and the
     * value is the total number of laptops in that category.
     * </p>
     *
     * @return a {@code HashMap} containing category names as keys and their
     *         corresponding laptop counts as values, or {@code null} if an error
     *         occurs
     */
    @Override
    ...
    /**
     * Counts the number of laptops that are either low stock or out of stock.
     *
     * @return the total number of laptops with availability status
     *         {@code LOW STOCK} or {@code OUT OF STOCK}, or {@code 0} if an error
     *         occurs
     */
    @Override
    ...
    /**
     * Calculates the total inventory valuation of all laptops.
     *
     * <p>
     * This method computes the sum of {@code stockQuantity * price} for all
     * laptop records in the database.
     * </p>
     *
     * @return the total valuation of all laptops in stock, or {@code 0} if an error
     *         occurs
     */
    @Override
     ...
     *
     *         <p>
     *         If the provided value is {@code null} or blank, this method returns
     *         the wildcard {@code "%"}, which matches any value. Otherwise, the
     *         value
     *         is converted to lowercase and wrapped with {@code %} on both sides to
     *         enable partial matching in SQL queries.
     *         </p>
     */
     ...
     *           data
     * @return a fully populated {@code Laptop} object, or {@code null} if a
     *         {@link SQLException} occurs during mapping
     * @throws Exception no exception is thrown directly, but any
     *                   {@link SQLException} encountered is caught and logged
     * @author Sugam Rana Magar Maps a {@link ResultSet} row to a {@code Laptop}
     *         entity.
     *         <p>
     *         This method extracts column values from the provided
     *         {@code ResultSet} and constructs a new {@code Laptop} object. It is
     *         used to reduce redundancy by centralizing the mapping logic between
     *         the database table and the Java entity.
     *         </p>
     *         <p>
     *         All relevant fields such as specifications, pricing, images, and
     *         metadata (created and updated timestamps) are retrieved from the
     *         result set.
     *         </p>
     * @implNote This method assumes that all required database columns exist and
     *           contain valid, non-null values for timestamp conversion.
     */
    // This method reduces the redundancy in code while assign the db table to java
    // entity
